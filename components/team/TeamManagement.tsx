...
import { useTeamMembersSWR, useTasksSWR } from '@/lib/hooks/useSWR';
import { TeamMember } from '@/lib/supabase/types';
import { supabase } from '@/lib/supabaseClient';

const TeamManagement = memo(() => {
  const { data: teamMembers = [], isLoading: loading, error, mutate: refetch } = useTeamMembersSWR();
  const { data: tasks = [] } = useTasksSWR();
  ...
  if (loading) {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-8 text-center">
        <div className="animate-pulse text-sm text-slate-600">Loading team data...</div>
      </div>
    );
  }
  ...
  if (!Array.isArray(teamMembers)) {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-8 text-center">
        ...
        <p className="text-sm text-slate-600 mb-4">Team data format is invalid. Please refresh the page.</p>
        <button
          onClick={() => window.location.reload()}
          className="btn-primary"
        >
