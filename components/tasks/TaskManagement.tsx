...
import { 
  ...
  Calendar, Target, MessageSquare, RefreshCw, ChevronDown, ChevronUp
} from 'lucide-react';
import { useTasksSWR, useTeamMembersSWR, useVehiclesSWR } from '@/lib/hooks/useSWR';
import { Task } from '@/lib/supabase/types';
import { supabase } from '@/lib/supabaseClient';

const TaskManagement = memo(() => {
  const { data: tasks = [], isLoading: loading, error, mutate: refetch } = useTasksSWR();
  const { data: teamMembers = [] } = useTeamMembersSWR();
  const { data: vehicles = [] } = useVehiclesSWR();
  ...
  // Optimized task operations with SWR mutations
      ...
      <div className="space-y-6">
        <div className="bg-white border border-slate-200 rounded-lg p-8 text-center">
          <div className="flex items-center justify-center space-x-3">
            ...
            <span className="text-sm text-slate-600">Loading task management...</span>
          </div>
        </div>
      </div>
    );
  ...
  if (!Array.isArray(tasks)) {
    return (
      <div className="bg-white border border-slate-200 rounded-lg p-8 text-center">
        ...
        <p className="text-sm text-slate-600 mb-4">Task data format is invalid. Please refresh the page.</p>
        <button
